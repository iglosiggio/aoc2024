...
Los resultados parciales me dan una secuencia monotónicamente creciente. Hacer
backtracking con poda debería ser suficientemente eficiente (hay como mucho 10
números por línea en el input grande)
...
let parse(data) = (
  data
    ...
    .filter(is_neq(""))
    .map(v => {
      let (goal, numbers) = v.split(": ")
      goal = int(goal)
      numbers = numbers.split().map(int)
      return (goal: goal, numbers: numbers)
    }))
...
Ok, con eso leo los datos, ahora la función que cuenta la cantidad de formas:
```definition
let soluciones(goal, numbers, i, acc) = {
  // Si me pasé de rosca no hay soluciones
  if goal < acc {
    return 0
  }
  // Si no me quedan decisiones por tomar entonces me fijo si lo resolví
  if i == numbers.len() {
    if goal == acc {
      return 1
    } else {
  ...
  let n = numbers.at(i)
  return (
    soluciones(goal, numbers, i + 1, acc + n)
    + soluciones(goal, numbers, i + 1, acc * n))
}
...
let answer = 0
for problem in data {
  let x = soluciones(problem.goal, problem.numbers, 0, 0)
  dbg(problem: problem, soluciones: x)
  if 0 < x {
    answer = answer + problem.goal
  }
}
dbg(answer: answer)
```

Ok, y si lo pruebo con la data posta? (sí, me acabo de dar cuenta que leí mal
el enunciado y alcanzaba con ver si era resoluble o no)

```repl
let data = parse(read("2024-12-07.data"))
let answer = 0
for problem in data {
  if 0 < soluciones(problem.goal, problem.numbers, 0, 0) {
    answer = answer + problem.goal
  }
}
dbg(answer: answer)
...
```definition
let tiene-solucion(goal, numbers, i, acc) = {
  // Si me pasé de rosca no hay soluciones
  if goal < acc {
    return false
  }
  ...
  let n = numbers.at(i)
  return (
    tiene-solucion(goal, numbers, i + 1, acc + n)
    or tiene-solucion(goal, numbers, i + 1, acc * n)
    or tiene-solucion(goal, numbers, i + 1, acc * calc.pow(10, calc.floor(calc.log(n) + 1)) + n))
}
(tiene-solucion: tiene-solucion)
```
...
let answer = 0
for problem in data {
  let x = tiene-solucion(problem.goal, problem.numbers, 0, 0)
  dbg(problem: problem, tiene-solucion: x)
  if x { answer = answer + problem.goal }
}
dbg(answer: answer)
```
...
```repl
let answer = (
  parse(read("2024-12-07.data"))
  .filter(v => tiene-solucion(v.goal, v.numbers, 0, 0))
  .map(field("goal"))
  .sum())
dbg(answer: answer)
```
